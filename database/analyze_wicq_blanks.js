...
console.log(`Found ${blankGrRows.length} blank GR rows in WICQ`);

// Let's sort by value
blankGrRows.sort((a, b) => b['Total Value'] - a['Total Value']);

blankGrRows.forEach(r => {
  console.log(`Row ${r.rowIndex} mat=${r.Material} val=${r['Total Value']} entered=${r['Entered On']} transfer=${r['Aging(Last Date of Trnsfr)']} mfg=${r['Manufacture Date Aging']} mfg_date=${r['Manufacture Date']}`);
});
