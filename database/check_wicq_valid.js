...
for (let r = headerRow + 1; r <= range.e.r; r++) {
  ...
  if (loc === 'WICQ') {
    ...
    if (grCell && grCell.v) {
      ...
      validGrRows.push(rowObj);
    }
  }
}

console.log(`Found ${validGrRows.length} valid GR rows in WICQ:`);
validGrRows.forEach(r => {
  console.log(`Row ${r.rowIndex} mat=${r.Material} val=${r['Total Value']} AA=${r.AA} aging=${r['Aging(Date of Rcpt)']} gr=${r['GR Issue date']}`);
});
